...
begin
  if pg_catalog.cardinality(v_browser_functions)<>7
    ...
    raise exception 'Required browser RPC signatures are incomplete.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Direct API table or column privileges remain.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Inherited API table privileges remain.';
  end if;
  ...
  if exists (
    ...
    raise exception 'An API-facing table policy remains.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Direct API sequence privileges remain.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Inherited API sequence privileges remain.';
  end if;
  ...
  if exists (
    ...
    raise exception 'A PUBLIC function grant remains.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Unexpected API function grants remain.';
  end if;
  ...
  foreach v_browser_function in array v_browser_functions loop
    if not pg_catalog.has_function_privilege('anon',v_browser_function,'EXECUTE')
      ...
      raise exception 'A required browser RPC grant is missing.';
    end if;
  end loop;
end;
$no_public_table_access$;
